...
function parseHbsFile(file, gettextItems) {
...
function parseHbs(fileName, astBody, gettextItems = []) {
  ...
  filterAstItemsByPath(filteredItems, ['t']).forEach((item) => {
    ...
    if (item.params.length === 2) {
      gettextItem.messageContext = getItemText(item.params[1], fileName);
    }
  ...
  filterAstItemsByPath(filteredItems, ['n']).forEach((item) => {
    if (item.params.length < 3 || item.params.length > 4) {
      throw new Error(
    ...
    if (item.params.length === 4) {
      gettextItem.messageContext = getItemText(item.params[3], fileName);
    }
